 ...
 * @LastEditTime: 2023-05-23 03:09:33
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        delete require.cache[require.resolve(findCommandFile(`./${command.data.name}.js`))];

        try {
            interaction.client.commands.delete(command.data.name);
            const newCommand = require(findCommandFile(`./${command.data.name}.js`));
...
function findCommandFile(commands_name) {
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            const foundFile = findCommandFile(fileName, filePath);
            if (foundFile) {
                return foundFile;
            }
