 ...
 * @LastEditTime: 2023-05-22 06:11:38
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (day_limit !== null) {
            ...
            fourteenDaysAgo.setDate(fourteenDaysAgo.getDate() - 100);
        }
        ...
        if (messagesToDeleteWithin14Days.length > 0) {
            ...
            channel.bulkDelete(messagesToDeleteWithin14Days)
                .then(deletedMessages => {
                    ...
                    interaction.replay(`成功删除 ${deletedMessages.size} 條消息`);
                })
                .catch(error => {
                    console.error('刪除訊息時發生錯誤', error);
                    interaction.replay('刪除訊息發生錯誤', error);
                    interaction.users.send(notifyId, `刪除訊息發生錯誤，錯誤訊息：${error}`);
                });
        }
    },
};