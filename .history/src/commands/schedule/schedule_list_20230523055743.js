 ...
 * @LastEditTime: 2023-05-23 05:57:43
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        data.choices.forEach(async (u, index) => {
            ...
            if ((Option_Time === false || currentData.choices_hour === Option_Time) &&
                (Option_User === interaction.user || currentData.choices_name_id === Option_User.id) &&
                (Option_Channel === null || currentData.choices_channel === Option_Channel.id) &&
                (Option_String === null || currentData.choices_schedule.includes(Option_String)) &&
                (Option_Tag === null || currentData.choices_tag === Option_Tag)) {
                Chose_Date_Index.push(i);
            }
        ...
        for (let i = 0; i < Chose_Date.length; i++) {
            schedule_Embed.addFields({
                ...
                value: `提醒使用者：${Chose_Date[i].choices_name}
                        ...
                        是否標記：${true_false_to_zn_tw(Chose_Date[i].choices_tag)}`
            });
        }
