 ...
 * @LastEditTime: 2023-05-21 17:37:08
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        collector.on('end', collected => {
            ...
            const hasSelectedDayButton = collected.has('true_button_select_day');
            if (collector.ended) {
                if (hasSelectedDayButton) {
                    if (DebugMode === true) {
                        ...
                        interaction.editReply({ content: `該指令已結束，如需設定其他提醒請重新輸入\`/schedule \`。`, components: [], ephemeral: true }).catch(console.error);
                    }
                }
            }
            if (collector.ended) {
