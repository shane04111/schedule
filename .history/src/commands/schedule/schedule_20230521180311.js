 ...
 * @LastEditTime: 2023-05-21 18:03:11
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // 自動添加小時
        const collector = await response.createMessageComponentCollector({ componentType: ComponentType.Button, time: 15_000 });
        const collector_select = await response.createMessageComponentCollector({ componentType: ComponentType.StringSelect, time: 15_000 });
        ...
        collector.on('end', collected => {
            ...
            const hasSelectedDayButton = collected.has('true_button_select_all');
            console.log(hasSelectedDayButton);
            if (collector.ended) {
                if (DebugMode) {
                    ...
                    if (hasSelectedDayButton) {
                        interaction.editReply({ content: `該提醒已安排，如需設定其他提醒請重新輸入\`/schedule \`。`, components: [], ephemeral: true }).catch(console.error);
                    } else {
