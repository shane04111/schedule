 ...
 * @LastEditTime: 2023-05-24 08:37:18
...
const { CreateComponent } = require(`../../function/CreateComponents.js`);
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const file = interaction.options.getAttachment('檔案') ?? 'NA';

        if (!schedule_say && file === 'NA') {
            await interaction.reply('請至少填寫其中一個選項。');
            return;
        }
        ...
        const Button_row = new ActionRowBuilder()
            .addComponents(
                Component.button_now_row,
                Component.button_year_and_month_row,
                Component.button_year_row,
                Component.button_nothing_row
            );

        // 年份選取合併
        const true_false_in_one_row_year = new ActionRowBuilder()
            .addComponents(
                Component.true_button_select_year_user,
                Component.false_button_select_year_user
            );

        // 月份選取合併
        const true_false_in_one_row_month = new ActionRowBuilder()
            .addComponents(
                Component.true_button_select_month_user,
                Component.false_button_select_month_user
            );

        // 日期選取合併
        const true_false_in_one_row_day = new ActionRowBuilder()
            .addComponents(
                Component.true_button_select_day_user,
                Component.false_button_select_day_user
            );

        // 小時選取合併
        const true_false_in_one_row_hour = new ActionRowBuilder()
            .addComponents(
                Component.true_button_select_hour_user,
                Component.false_button_select_hour_user
            );

        // 分鐘選取合併
        const true_false_in_one_row_minute = new ActionRowBuilder()
            .addComponents(
                Component.true_button_select_minute_user,
                Component.false_button_select_minute_user
            );

        // 時間點選取合併
        const true_false_in_one_row_all = new ActionRowBuilder()
            .addComponents(
                Component.true_button_select_all_user,
                Component.false_button_select_all_user
            );

        const Day_time_button = new ActionRowBuilder()
            .addComponents(
                Component.Day_time_up_button,
                Component.Day_time_down_button
            );
        ...
        collector.on('collect', async i => {
            ...
            switch (buttonId) {
                case 'button_nothing':
                    await i.update({ content: '請選擇提醒時間', components: [Select_Menu_year, Component.back_Button_nowadays], ephemeral: true });
                    break;
                case 'button_year':
                    ...
                    await i.update({ content: '請選擇提醒時間', components: [Select_Menu_Month, Component.back_Button_nowadays], ephemeral: true });
                    break;
                ...
                case 'button_now':
                    ...
                    await i.update({ content: '請選擇幾點提醒', components: [Select_Menu_Hour, Component.back_Button_nowadays], ephemeral: true });
                    break;
                case 'Back_Button':
